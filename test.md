        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-6">
          ...
          {/* Rajasthan Map */}
          <div className="lg:col-span-12 bg-white rounded-lg shadow-sm border border-gray-200 p-5 flex flex-col">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="overflow-x-auto min-h-[300px] relative">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-[12px] font-semibold text-gray-700">
                {grievances.length > 0 ? grievances.map((row, idx) => (
                  <tr key={row._id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors group">
                    <td className="px-5 py-3 text-gray-400 font-bold">{idx + 1}</td>
                    <td className="px-5 py-3 text-[#002b5e] font-bold">{row.complainId}</td>
                    <td className="px-5 py-3">
                      <div className="flex flex-col">
                        <span className="text-gray-900 leading-tight">{row.geographic_information?.district}</span>
                        <span className="text-[9px] text-gray-400 uppercase tracking-wider">{row.geographic_information?.block}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                      <span className="px-1.5 py-0.5 bg-gray-100 rounded text-[10px] text-gray-500 font-bold uppercase">{row.case_specifics?.department}</span>
                    </td>
                    <td className="px-5 py-3">
                      <p className="max-w-[200px] truncate text-gray-500 font-bold">{row.complain_profile?.complainer?.name}</p>
                    </td>
                    <td className="px-5 py-3 text-right">
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                        row.enforcement_status?.case_status === 'Pending' ? 'bg-orange-100 text-orange-700' : 
                        row.enforcement_status?.case_status === 'Resolved' ? 'bg-green-100 text-green-700' : 
                        'bg-red-100 text-red-700'
                      }`}>
                        {row.enforcement_status?.case_status}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-center">
                      <button 
                        onClick={() => navigate(`/track/${row.complainId}`)}
                        className="text-teal-600 hover:text-teal-800 font-bold text-[10px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        Track
                      </button>
                    </td>
                  ...
                  <tr>
                    <td colSpan="7" className="px-5 py-10 text-center text-gray-400 italic">
                      {loading ? 'Fetching records...' : 'No grievances found matching your criteria.'}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="p-3 bg-gray-50/50 border-t border-gray-100 flex justify-between items-center">
             <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{grievances.length} Records Shown</span>
